#
#
#   A class to handle SAX events, specifically for a NeuroML2 file.
...
class NeuroML2SaxHandler(xml.sax.ContentHandler):
  ...
  def startElement(self, name, attrs):
    ...
    if name == 'neuroml':     
      self.log.debug("Found main neuroml element")
      #self.log.debug("lengthUnits: "+ attrs.get('lengthUnits',""))
    
    elif name == 'network':     
      self.log.debug("Found main network element")
      #self.log.debug("lengthUnits: "+ attrs.get('lengthUnits',""))
      
      
    elif name == 'include':
      self.log.debug("Found include element")
      ...
      self.currentProjectionName = attrs.get('id',"")   
      self.log.debug("Found projection element: "+ self.currentProjectionName)   

      if attrs.has_key('presynapticPopulation'):
          self.currentProjectionSource = attrs.get('presynapticPopulation',"") 
