...
public class PlotterFrame extends JFrame
{
    ...
    JMenuItem jMenuGenerateGnuplot = new JMenuItem();
    ...
    private void jbInit() throws Exception
    {
        ...
        this.jMenuGenerateGnuplot.addActionListener(new ActionListener()
        {
            public void actionPerformed(ActionEvent e)
            {
                jMenuGenerateGnuplot_actionPerformed(e);
            }
        });
...
/*
    ...
    void jMenuGenerateGnuplot_actionPerformed(ActionEvent e)
    {
        String lastDir = recentFiles.getMyLastExportPointsDir();
        ...
        chooser.setDialogTitle("Choose a location for the gnuplot files to generate the EPS image file");
        ...
        if (retval != JOptionPane.CANCEL_OPTION)
        {
            ...
            File gnuplotFilesDir = new File(defaultDir, plotRef);

            StringBuffer mainScript = new StringBuffer("# This is a gnuplot file generated by neuroConstruct version "+GeneralProperties.getVersionNumber()+"\n" +
                    "# This file will generate a plot.eps file when run using: \n\n" +
                    "#    gnuplot "+ProjectStructure.getGnuplotEpsFilename()+"\n\n" +
                    "# gnuplot is included with most standard Linux distributions or can  be installed from http://www.gnuplot.info\n\n");

            mainScript.append("set term postscript eps color blacktext \"Arial\" 24\n");
            mainScript.append("set output \""+plotRef+".eps\"\n\n");

            gnuplotFilesDir.mkdir();

            //Hashtable<String, String> dataVsLoadScript = new Hashtable<String, String>();


            mainScript.append("plot ");

            for (int j=0;j<plotCanvas.getDataSets().length;j++)
            {
                DataSet ds = plotCanvas.getDataSets()[j];
                File dsFile = new File(gnuplotFilesDir, getCompactFilename(ds.getRefrence())+".dat");
                ...
                try
                {
                    ...
                    float maxY = (float)ds.getMaxY()[1];
                    float minY = (float)ds.getMinY()[1];
                    float resolution = (maxY-minY)/3000;

                    float lastYPlotted = Float.NaN;

                    for(int i=0;i<ds.getNumberPoints();i++)
                    {
                        double[] p = ds.getPoint(i);
                        if (Float.isNaN(lastYPlotted) || Math.abs(lastYPlotted - (float)p[1])>resolution)
                        {
                            fw.write((float)p[0]+" "+(float)p[1]+"\n");
                            lastYPlotted = (float)p[1];
                        }
                    }
                    ...
                    mainScript.append(" \""+name+"\"  with linespoints  lt rgb \"#"+rgb+"\" lw 1 pt 0 ");

                }
                catch (IOException ex)
                {
                    GuiUtils.showErrorMessage(logger, "Error creating file: "+dsFile
                        +" for generating EPS with gnuplot", ex, this);
                    return;

                }
            }
            mainScript.append("\n");
            mainScript.append("set output\n");
            mainScript.append("quit\n");


            File mainGnuplotFile = new File(gnuplotFilesDir, ProjectStructure.getGnuplotEpsFilename());

            logger.logComment("Going to write file: "+mainGnuplotFile);
            try
            {
                FileWriter fw = new FileWriter(mainGnuplotFile);
            ...
            {
                GuiUtils.showErrorMessage(logger, "Error creating file: "+mainGnuplotFile
                    +" for generating EPS with gnuplot", ex, this);
                return;
